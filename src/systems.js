import {Ball, AimLine, BoxTile, BallPowerUp, LevelDialog} from './renderers';
...
function promptDialog(entities) {
  const {scorebar} = entities;
  if (scorebar.state != Config.PAUSED) {
    entities['dialog'] = {
      message: 'You win!',
      renderer: LevelDialog,
    };
    scorebar.state = Config.PAUSED;
  } else {
    scorebar.state = Config.STOPPED;
